...
struct kills_spacket { /* SP_KILLS py-struct "!bbxxI" #3 */
    ...
    char pad1;
    char pad2;
    unsigned int kills;	/* where 1234=12.34 kills and 0=0.00 kills */
};
...
struct ping_spacket { /* SP_PING py-struct "!bBHBBBB" #46 */
   ...
   char		tloss_sc;       /* total loss server-client 0-100% */
   char		tloss_cs;       /* total loss client-server 0-100% */

   char		iloss_sc;       /* inc. loss server-client 0-100% */
   char		iloss_cs;       /* inc. loss client-server 0-100% */
...
struct torp_cpacket { /* CP_TORP py-struct "!bBxx" #6 */
    char type;
    char dir;		/* direction to fire torp */
    char pad1;
    char pad2;
};
...
/*struct younsigned short_spacket {       /* SP_S_YOU *//*
     char       type;
...
struct planet_s_spacket {       /* body of SP_S_PLANET  */
    ...
    char owner;
    char info;
    char armies;       /* more than 255 Armies ? ...  */
    short flags;
};
...
struct player_s_body_spacket {  Body of new Player Packet
        char pnum;      0-4 = pnum, 5 local or galactic, 6 = 9. x-bit, 7 9. y-b
it
        char speeddir;  0-3 = speed , 4-7 direction of ship
        char x;         low 8 bits from X-Pixelcoordinate
        char y;         low 8 bits from Y-Pixelcoordinate
};
*/

struct torp_s_spacket {
     char type; /* SP_S_TORP */
     char bitset;      /* bit=1 that torp is in packet */
     char whichtorps; /* Torpnumber of first torp / 8 */
     char data[21]; /* For every torp 2*9 bit coordinates */
};
...
struct mesg_s_spacket {
    ...
    char m_recpt;
    char m_from;
    char length;       /* Length of whole packet */
    char mesg;
    char pad2;
    char pad3;
    char pad[76];
...
struct phaser_s_spacket
  {
    ...
    char status;                /* PH_HIT, etc... */
    char pnum;                 /* both bytes are used for more */
    char target;               /* look into the code   */
    short x;                    /* x coord /40 */
    short y;                    /* y coord /40 */
    char dir;
    char pad1;
...
struct generic_32_spacket_b { /* SP_GENERIC_32 py-struct "!b1sHbHBBsBsBB18x" #32 */
    ...
    char      tournament_teams;        /* what teams are involved         */
    char      tournament_age;          /* time since last t-mode start    */
    char        tournament_age_units;    /* units for above, see s2du       */
    char      tournament_remain;       /* remaining INL game time         */
    char        tournament_remain_units; /* units for above, see s2du       */
    char      starbase_remain;         /* starbase reconstruction, mins   */
    char      team_remain;             /* team surrender time, seconds    */
    char        pad1[18];
